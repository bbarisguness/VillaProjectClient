...
export default function Reservation() {
    ...
    useEffect(() => {
        ...
        setreservationItems(JSON.parse(localStorage.getItem('reservation')) || null)
    }, [])
    ...
    useEffect(() => {
        if (reservationItems) {
            ...
            if ((Math.floor(new Date().getTime() / 1000) - reservationItems.expiryDate) > 3600) {
            ...
            fetch(`${process.env.NEXT_PUBLIC_API_URL}/villas/${reservationItems.villaId}?${query}`)
                .then(res => res.json())
                .then(
                    (result) => {
    ...
    const getDateString = (girismiCikismi = 'g') => {
        ...
        if (reservationItems.length != 0) {
            let dateArray = girismiCikismi == 'g' ? reservationItems.checkIn.split('-') : reservationItems.checkOut.split('-')
        ...
        if (transferType == 1) {
            ...
            getPrice({
                ...
                if (!res) {
                    ...
                    createReservation(reservationData, personData, reservationItems.reservationItems, transferType, villa?.attributes?.name).then((res) => {
                        if (res)
                            setActiveStep(2)
                        else
                            setActiveStep(0)
    ...
    return (
        <>
            <Seo pageTitle={'Rezervasyon Oluştur'} pageDesc={'Labirent Fethiye Rezervasyon Oluşturma'} />
                                                        ...
                                                        <ul>
                                                            ...
                                                            <li>
                                                                <div className={styles.inputBox}>
                                                                    ...
                                                                    <input name="idNo" value={values.idNo} onChange={handleChange} type="text" placeholder="•••••" minLength="2" maxLength="30" />
                                                                    {
                            ...
                            {activeStep !== 2 &&
                                <div className={styles.right}>
                                    <div className={styles.top}>
                                        <div className={styles.imageBox}>
                                            <div className={styles.img} style={{ backgroundImage: villa?.attributes && `url(${villa?.attributes?.photos?.data[0]?.attributes?.photo?.data?.attributes?.formats?.thumbnail?.url})` }}></div>
                                        </div>
                                        <div className={styles.textBox}>
                                            <div className={styles.title}>{villa?.attributes?.name}</div>
                                            <div className={styles.desc}>{villa?.attributes?.region}</div>
                                        </div>
                                    </div>
                                        ...
                                        <ul>
                                            <li>
                                                <div className={styles.visitorNumberBox}>
                                                    <div className={styles.visitorBox}>
                                                        <span>{reservationItems && `${reservationItems.adult} Yetişkin, ${reservationItems.child} Çocuk, ${reservationItems.baby} Bebek`}</span>
                                                    </div>
                                            ...
                                            <li>
                                                <div className={styles.priceBox}>
                                                    <span>{reservationItems?.reservationItems?.length} Gece </span>
                                                    <span>{[reservationItems?.totalPrice].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")}₺</span>
                                                </div>
                                                {/* <div className={styles.priceBox}>
                                        <span>3715 ₺ <i></i></span>
                                        <span>400 ₺</span>
                                            ...
                                            <li>
                                                <div className={styles.priceBox}>
                                                    <span>Ön Ödeme</span>
                                                    <span>{(reservationItems.totalPrice * 30 / 100).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")} ₺</span>
                                                </div>
                                                <div className={styles.priceBox}>
                                                    <span>Girişte Ödeme</span>
                                                    <span>{(reservationItems.totalPrice - (reservationItems.totalPrice * 30 / 100)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")} ₺</span>
                                                </div>
                                                <div className={styles.priceBox}>
                                                    <span>Toplam</span>
                                                    <span><strong>{[reservationItems?.totalPrice].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")} ₺</strong></span>
                                                </div>
                                            </li>
                                        </ul>
                                    </div>
